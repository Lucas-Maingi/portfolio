...
export default function Projects() {
  const projects: Project[] = [
    ...
    },
    {
      ...
      problem: "On-call is exhausting because the same incidents recur. SentryBrain streams server telemetry, uses an LLM to diagnose root causes and propose fixes, and puts two walls between the model and your shell: a deterministic command policy (denylist + default-deny allowlist, enforced at proposal AND execution — an approved `rm -rf /` still doesn't run) plus human approval. Then it verifies its own fix before closing the incident.",
    ...
    },
    {
      ...
      problem: "Every LLM feature has an unguarded seam: raw input goes up, raw output comes down. Aegis Shield is a drop-in proxy that scans both directions for PII leaks, prompt injections, and secret exposure — OpenAI-compatible, one base_url override, with caching and rate limiting. Full inbound scanning adds ~1.6ms p50 (measured, methodology in the repo).",
      tech: ["Python", "FastAPI", "SQLite", "Streamlit", "Docker"],
      metric: "~1.6ms",
      metricLabel: "Measured scan overhead (p50)",
